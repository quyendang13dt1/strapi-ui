...
export class ArticleDetailComponent {
  ...
  ngOnInit(): void {
    ...
    combineLatest([
      ...
      this._ArticleService.getArticleDetailRelation(
        this.id,
        this._ActivatedRoute.snapshot.queryParamMap.get('status')
      ),
        ...
        ?.map((part: any) => {
          return {
            ...
            data: part
              .replace(
                /<oembed url="(.*?)"><\/oembed>/g,
                `<iframe style="  width: 100%; max-width: 800px;height: 450px; border: none;" src="$1" frameborder="0" allowfullscreen></iframe>`
              )
